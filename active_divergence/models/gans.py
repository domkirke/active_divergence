...
from active_divergence.models import Model
...
class GAN(Model):
    ...
    def sample_prior(self, batch=None, shape=None):
        if batch is not None:
            ...
            z = self.prior((*batch.shape[:batch_len], *self.generator.input_size)).sample()
        else:
    ...
    def discriminator_loss(self, discriminator, batch, generation, d_real, d_fake, drop_detail=False):
    ...
    def generator_loss(self, generator, batch, out, d_fake, z, hidden=None, drop_detail=False):
        if self.config.training.mode == "adv":
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self._loss_phase == 0 or self._loss_phase is None:
            ...
            d_loss, d_losses = self.discriminator_loss(self.discriminator, batch, out_disc, d_real, d_fake, drop_detail=True)
            loss = loss + d_loss
        ...
        if self._loss_phase == 1 or self._loss_phase is None:
            if self.config.training.get('feature_matching'):
                ...
                g_loss, g_losses = self.generator_loss(self.generator, batch, out, d_fake, z, hidden=[hidden_true, hidden_fake], drop_detail=True)
            else:
                d_fake = self.discriminate(out, z, return_hidden=False)
                g_loss, g_losses = self.generator_loss(self.generator, batch, out, d_fake, z, drop_detail=True)
            loss = loss + g_loss
...
class ProgressiveGAN(GAN):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        d_loss = self.discriminator_loss(self.current_discriminator, batch, out, d_real, d_fake)
        # get generator loss
        g_loss = self.generator_loss(self.current_generator, batch, out, d_fake, z, hidden=[hidden_real, hidden_fake])
        if g_loss is not None:
