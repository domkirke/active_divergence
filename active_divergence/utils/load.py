...
from active_divergence.utils.save import get_model_versions
...
def load_vae(name: str, root: str = "models/vae/"):
    ...
    for t in range(len(transform.transforms)):
        if isinstance(transform[t], at.STFT):
            transform.transforms[t].inversion_mode = "griffin_lim"
        if isinstance(transform[t], at.DGT):
            transform.transforms[t].inversion_mode = "griffin_lim"
    model.eval()
...
def load_diffwave(name: str, root: str = "models/diffwave"):
    model_path = os.path.join(root, name+".pt")
    config = OmegaConf.load(os.path.join(root, name+".yaml"))
...
def load_ddsp(name: str, root: str = "models/ddsp"):
    model_path = os.path.join(root, name+".pth")
    config = OmegaConf.load(os.path.join(root, name+".yaml"))
